...
class Tab:
    def __init__(self, name, x, y , tab_width = TAB_WIDTH, tab_height = TAB_HEIGHT , text = True):
        self.name = name
    ...
    def draw(self, screen, font,img=True):
        ...
        # 绘制标签背景（带边框效果）
        pygame.draw.rect(screen, SHADOW, (self.rect.x , self.rect.y , self.rect.width, self.rect.height))
        ...
        if img:
            render_1bit_sprite(screen, load_image(f"arts/sprite/{self.name}.png",(16,16)), (self.rect.x + 3 , self.rect.y + 3), text_color)
...
class PageContainer(Page):
    ...
    def register_page(self, page , tab_width = TAB_WIDTH ,tab_height = TAB_HEIGHT , text = True):
        x, y = self.tab_pos
        ...
        tab = Tab(page.name, tab_x, tab_y ,tab_width = tab_width ,tab_height = tab_height, text = text)
...
class CharacterPage(Page):
    ...
    def draw(self, screen, font, player=None):
        ...
        for i, (key, val) in enumerate(data['base_stats'].items()):
            ...
            screen.blit(stat_surface, (200, start_y + i * line_height))

        self.draw_status(screen,content_font,250,50,data['status'])
    ...
    def draw_status(self,screen, font, x, y, status_list):
        ...
        for body_part, illnesses in categorized.items():
            ...
            for illness in illnesses:
                ...
                screen.blit(illness_text, (x + 10, current_y))
                current_y += font.get_height() + 2
...
class Toolbar(PageContainer):
    def __init__(self, get_player_data):
        super().__init__("Toolbar", get_player_data, tab_pos=(SCREEN_WIDTH - 30, SCREEN_HEIGHT - 130 ), direction="col")
        from AbilityTree import AbilityPage
        self.register_page(AbilityPage(get_player_data),text = False)
        self.register_page(InventoryPage(),text = False)
